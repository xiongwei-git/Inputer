package com.mrxiong.argot.window;
...
public class TWindowManager {
  ...
  private static LayoutParams makeFloatBtnParams(Context context) {
    SharePreData sharePreData =
        new FavorAdapter.Builder(context).build().create(SharePreData.class);
    ...
    if (lastX < 0 || lastY < 0) {
      ...
      lastX = screenWidth / 2
          - context.getResources().getDimensionPixelSize(R.dimen.fab_size_normal) / 2;
  ...
  public static void hideRevertBtn() {
    if (mFloatBtnView != null && mFloatBtnView.getFloatBtnType()
        .equals(FloatWindowView.FloatBtnType.BACK)) {
      hideFloatBtn();
    }
  }
   ...
   */
  @Deprecated public static boolean isWindowShowing() {
